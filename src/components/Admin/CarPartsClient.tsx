...
interface Props {
  initialParts: CarPartInterface[];
}

/**
 * Client island for the car-parts admin page.
 *
 * (Day 12.6 / Finding #29) Initial inventory is fetched server-side by
 * the parent page so the first paint already shows the table — no
 * loading spinner, no extra network round-trip. Create/edit/delete
 * mutations refetch the list to keep the table in sync.
 *
 * Sibling of ViewingBookingsClient + ServiceBookingsClient.
 */
export default function CarPartsClient({ initialParts }: Props) {
  const [parts, setParts] = useState<CarPartInterface[]>(initialParts);
  ...
  // Single submit handler — POSTs for new parts, PUTs for edits.
  ...
  return (
    <div className="p-6">
      ...
      {/* Delete Confirmation — shared ConfirmDialog primitive */}
      {deleteTarget && (
        <ConfirmDialog
