...
test.describe("GDPR / cookie consent", () => {
  ...
  test("cookie banner is visible on first visit", async ({ context, page }) => {
    ...
    // Look for any consent banner / dialog with common patterns.
    const banner = page
      .locator("[data-testid=cookie-banner], [role=dialog]:has-text('cookie'i), :text('cookies')")
      .first();
    const found = await banner.isVisible().catch(() => false);
    console.log(`[cookie banner visible] ${found}`);
    expect(found, "cookie banner should appear before consent recorded").toBe(true);
  });
